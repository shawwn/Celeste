...
namespace Celeste
{
  public static class Sfxs
  {
    ...
    public const string char_mad_hiccup_standing = "event:/new_content/madeline/hiccup_standing";
    public const string char_mad_hiccup_ducking = "event:/new_content/madeline/hiccup_ducking";
    ...
    public const string game_09_finalheartget = "event";
    ...
    {
      foreach (FieldInfo field in typeof (Sfxs).GetFields(BindingFlags.Static | BindingFlags.Public))
      {
        if (field.FieldType == typeof (string))
        {
          string key = field.GetValue((object) null).ToString();
          Sfxs.byHandle.Add(field.Name, key);
          if (key.StartsWith("event:/char/madeline/"))
            Sfxs.MadelineToBadelineSound.Add(key, key.Replace("madeline", "badeline"));
        }
      }
      Sfxs.MadelineToBadelineSound.Add("event:/game/general/assist_screenbottom", "event:/game/general/assist_screenbottom");
    }

    public static string EventnameByHandle(string handle)
    {
      string str = "";
      Sfxs.byHandle.TryGetValue(handle, out str);
      return str;
    }
  }
}
