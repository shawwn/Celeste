...
namespace Celeste
{
  ...
  public class Debris : Actor
  {
    ...
    public Debris Init(Vector2 pos, char tileset)
    {
      ...
      this.image.Scale.X = (__Null) (double) Calc.Random.Range(0.5f, 1f);
      this.image.Scale.Y = (__Null) (double) Calc.Random.Range(0.5f, 1f);
    ...
    public Debris BlastFrom(Vector2 from)
    {
      ...
      this.speed = Vector2.op_Subtraction(this.Position, from).SafeNormalize(length);
    ...
    private void OnCollideV(CollisionData data)
    {
      ...
      if (this.speed.Y < 0.0 && this.speed.Y > -50.0)
        this.speed.Y = (__Null) 0.0;
      if (this.speed.Y == 0.0 && this.hasHitGround)
        return;
      this.ImpactSfx(Math.Abs((float) this.speed.Y));
    }
    ...
    {
      base.Update();
      this.image.Rotation += Math.Abs((float) this.speed.X) * (float) this.rotateSign * Engine.DeltaTime;
      ...
      if (this.dreaming)
      {
        this.speed.X = (__Null) (double) Calc.Approach((float) this.speed.X, 0.0f, 50f * Engine.DeltaTime);
        this.speed.Y = (__Null) (double) Calc.Approach((float) this.speed.Y, 6f * this.dreamSine.Value, 100f * Engine.DeltaTime);
      }
      else
      {
        bool flag = this.OnGround(1);
        this.speed.X = (__Null) (double) Calc.Approach((float) this.speed.X, 0.0f, (flag ? 50f : 20f) * Engine.DeltaTime);
        if (!flag)
          this.speed.Y = (__Null) (double) Calc.Approach((float) this.speed.Y, 100f, 400f * Engine.DeltaTime);
      }
      ...
      this.image.Color = Color.op_Multiply(Color.Lerp(Color.get_White(), Color.get_Gray(), this.fadeLerp), this.alpha);
    }
  }
}
